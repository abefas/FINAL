...
int main(int argc, char **argv) {
    ...
    if (NULL == (G.d_matrix = malloc(sizeof *G.d_matrix * G.n_nodes)) ||
        NULL == (G.t_matrix = malloc(sizeof *G.t_matrix * G.n_differentTypes))) {
        printf("Error mallocing d_matrix!\n");
        exit(1);
    }

    for(int i = 0; i < G.n_differentTypes; i++){
        if (NULL == (G.t_matrix[i] = malloc(sizeof *G.t_matrix[i] * G.n_nodes))) {
            printf("Error mallocing d_matrix[]!\n");
            exit(1);
        }
        for(int j = 0; j < G.n_nodes; j++){
            if (NULL == (G.t_matrix[i][j] = malloc(sizeof *G.t_matrix[i][j] * G.n_nodes))) {
                printf("Error mallocing d_matrix[]!\n");
                exit(1);
            }
        }
        ...
        for (int j = 0; j < G.n_nodes; j++) {
            if (i == j || (i >= G.n_customers && j >= G.n_customers)) {
                ...
                for(int t = 0; t < G.n_differentTypes; t++){
                    G.t_matrix[t][i][j] = G.d_matrix[i][j] / VT[t].speed;
                }
            }
        }
    }
    ...
    double T_update = 0.2; // The higher it is - the higher probability to update pheromones using the worse solution
    double a_update = 0.001;
    double p_min = 0.001;
    double p_max = 0.01;
    double d = 1.0;
    double a = 1.0;
