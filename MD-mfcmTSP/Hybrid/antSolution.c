...
void antSolution(SON *G, VType VT, vt_solution *Ra, int **K, double *phMatrix, 
    ...
    for(int idep = 0; idep < G->n_depots; idep++){
        if(G->a_depots[idep].n_VT[Ra->IVT] != 0){
            ...
            Ra->a_depots[idep].makespan = 
                get_makespan_depot_VT(G, Ra->a_depots[idep].routelist, G->a_depots[idep].n_VT[Ra->IVT], VT.IVT);
        }
    }
