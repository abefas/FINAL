    ...
    while(flagVT[0] || flagVT[1] || flagVT[2] || flag3){
        ...
        for (int ivt = 0; ivt < G->n_differentTypes; ivt++) {
            if (ivt != 2) {
                ...
                double og = R->a_VT[ivt].makespan;
                double ms = mutual_drone(&R->a_VT[ivt], G);
                if(ms < og - epsilon){
                    flagVT[ivt] = true;
                }else{
                    flagVT[ivt] = false;
        ...
        for(int ivt = 0; ivt < G->n_differentTypes; ivt++){
            ...
            for(int ivt2 = 0; ivt2 < G->n_differentTypes; ivt2++){
                ...
                while(l1_position < length_l1-n_max_t){                                   //For every node(s) in l1
                    ...
                    double ivt_ms = get_makespan_depot_VT(G, l1, G->a_depots[idep].n_VT[ivt], ivt);
                    ...
                    while(p && p->next){
                        ...
                        if(check == 0){
                            ...
                            double ivt2_ms = get_makespan_depot_VT(G, l2, G->a_depots[idep].n_VT[ivt2], ivt2);
...
double k_optimization2(route *r, SON *G, VType VT, int n_max){
    ...
    while(list_position < list_length-n_max){                           //For every node(s) in depotroute
        ...
        while(p && p->next){                    //While depotroute has nodes, place successive nodes after them
            ...
            if(check == 0){
                ms_new = get_makespan_depot_VT(G, depotroute, G->a_depots[idepot].n_VT[VT.IVT], VT.IVT);
    ...
    while(idep < G->n_depots){
        ...
        while(sec < G->n_depots){
            ...
            while(l1_position < length_l1-n_max){                                   //For every node(s) in l1
                ...
                double idep_ms = get_makespan_depot_VT(G, l1, G->a_depots[idep].n_VT[R->IVT], VT.IVT);
                bool flag = false;
                ...
                while(p && p->next){
                    ...
                    if(check == 0){
                        ...
                        double sec_ms = get_makespan_depot_VT(G, l2, G->a_depots[sec].n_VT[R->IVT], VT.IVT);
    ...
    while(idep < G->n_depots){
        ...
        while(sec < G->n_depots){
            ...
            while(l1_position < length_l1-1){                                   //For every node(s) in l1
                ...
                double idep_ms = get_makespan_depot_VT(G, l1, G->a_depots[idep].n_VT[R->IVT], R->IVT);
                double sec_ms = get_makespan_depot_VT(G, l2, G->a_depots[sec].n_VT[R->IVT], R->IVT);
