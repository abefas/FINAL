...
/*
void offload_to_motorbikes(SON *G, asolution *R_best, vt_solution *Rt_best, int **da_access, int m_capacity){
...
double get_makespan_depot_VT(SON *G, node *routelist, int n_vehicles, int ivt){
    ...
    node *p = routelist;
    while(p->next){
        route_time += G->t_matrix[ivt][p->data - 1][p->next->data - 1];
...
void update_pheromone(SON *G, vt_solution *Rt, vt_solution *Rt_best, asolution *R, asolution *R_best, double *phMatrix, double T_update, double d){
    ...
    for(int idep = 0; idep < G->n_depots; idep++){
        if(G->a_depots[idep].n_VT[0] != 0){
            if(flag){
                temp = Rt_best->a_depots[idep].routelist;
            }else{
                temp = Rt->a_depots[idep].routelist;
            }
            ...
            while(temp && temp->next){
                if(phMatrix[idep*G->n_nodes*G->n_nodes + (temp->data-1)*G->n_nodes + temp->next->data - 1] < 0.0){
                    ...
                    printf("routes:\nR:\n");
                    printList(R->a_VT[0].a_depots[idep].routelist);
                    printf("\nR_best:\n");;
                    printList(R_best->a_VT[0].a_depots[idep].routelist);
                    printf("\n");
...
double evaporate_pheromones(SON *G, double *phMatrix, int *edge_matrix, int edge_sum, int n_ants, double p_min, double p_max){
...
void fprint_results(asolution *R, SON *G, VType *VT, int **da_access){
    ...
    sprintf(file_name, "p%02d-%d-%d-%d_ND%d_run-%d.res", instance_id, (int)VT[0].speed, (int)VT[1].speed, (int)VT[2].speed, G->a_depots[0].n_VT[2], AACORUN);
    ...
    push(&vehicleRoute, 0);
    for(int ivt = 0; ivt < G->n_differentTypes; ivt++){
        sprintf(fn, "p%02d-%d-%d-%d_ND%d_run-%d_ivt_%d.csv", instance_id, (int)VT[0].speed, (int)VT[1].speed, (int)VT[2].speed, G->a_depots[0].n_VT[2], AACORUN, ivt);
        ...
        for(int idep = 0; idep < G->n_depots; idep++){
            if(G->a_depots[idep].n_VT[ivt] != 0){
                ...
                double v_ms = 0.0;
                while(temp->next){
                    v_ms += G->t_matrix[ivt][temp->data - 1][temp->next->data - 1];
                    if(temp->next->data > G->n_customers){
                        append(&vehicleRoute, 0);
                        fprintf(fp, "type %d  depot %d q_served %2d  route %2d  time %10.2lf\t", ivt+1, idep+G->n_customers+1, q_served, vehicle, v_ms);
                        fprintf(fp_1, "%d,%d\n", temp->data, temp->next->data);
...
void fprint_results_VT(vt_solution *R, SON *G, VType *VT, int *da_access){
    ...
    char file_name[40], fn[40];
    sprintf(file_name, "p%02d-%d-%d-%d_ND%d_run-%d_TRUCK.res", instance_id, (int)VT[0].speed, (int)VT[1].speed, (int)VT[2].speed, G->a_depots[0].n_VT[2], AACORUN);
    ...
    node *temp = NULL, *vehicleRoute = NULL;
    push(&vehicleRoute, 0);
    sprintf(fn, "p%02d-%d-%d-%d_ND%d_run-%d_TRUCK.csv", instance_id, (int)VT[0].speed, (int)VT[1].speed, (int)VT[2].speed, G->a_depots[0].n_VT[2], AACORUN);
    if(NULL == (fp_1 = fopen(fn, "w"))){
        perror("Error opening fp_1!\n");
        exit(1);
    }
...
void fprint_data_hybrid(VType *VT, int iterations, int best_iter, double foundtime, double runtime, double term_condition, int N_D){
    char file_name[40];
    sprintf(file_name, "p%02d-%d-%d-%d_ND%d_run-%d.data", instance_id, (int)VT[0].speed, (int)VT[1].speed, (int)VT[2].speed, N_D, AACORUN);
    FILE *fp;
    if(NULL == (fp = fopen(file_name, "w"))){
