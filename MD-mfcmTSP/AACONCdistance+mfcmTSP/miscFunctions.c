        ...
        if(G->a_depots[idep].n_VT[0] != 0){
            ...
            while(temp->next){
                ...
                if(temp->next->data > G->n_customers){
                    ...
                    fprintf(fp, "depot %d  route %d  q_s %d\t", idep+G->n_customers+1, vehicle, q_served);
                    fprintf(fp_1, "%d,%d\n", temp->data, temp->next->data);
        ...
        for(int idep = 0; idep < G->n_depots; idep++){
            if(G->a_depots[idep].n_VT[ivt] != 0){
                ...
                while(temp->next){
                    ...
                    if(temp->next->data > G->n_customers){
                        append(&vehicleRoute, 0);
                        fprintf(fp, "type %d  depot %d  route %d q_s %d  time %10.2lf\t", ivt+1, idep+G->n_customers+1, vehicle, q_served, v_ms);
                        fprintf(fp_1, "%d,%d\n", temp->data, temp->next->data);
                        fprintList(vehicleRoute, fp);
                        deleteList(&vehicleRoute);
                        vehicle++;	
