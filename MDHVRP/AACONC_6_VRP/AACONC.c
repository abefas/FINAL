            ...
            double T_update, double a_update, double p_min, double p_max, double d, double a, double b) 
{
        ...
        for (int t = 0; t < G->n_differentTypes; t++) {
            for (int k = 0; k < G->n_nodes; k++) {
                for (int l = 0; l < G->n_nodes; l++) {
                    // node to itself OR depot to depot or Depot doesn't support vehicle
                    // type
                    if (k == l || (k >= G->n_customers && l >= G->n_customers)) {
                        phMatrix[   d * G->n_differentTypes * G->n_nodes * G->n_nodes +
                                    t * G->n_nodes * G->n_nodes + 
                                    k * G->n_nodes + l] = -1.0;
                    } else {
                        if (t != 2) {
                            phMatrix[   d * G->n_differentTypes * G->n_nodes * G->n_nodes +
    ...
    time_t begin = time(NULL), loop = time(NULL),
    loop_time = difftime(loop, begin);

    char file_name[15];
    sprintf(file_name, "difference.res");
    FILE *fp;
    if (NULL == (fp = fopen(file_name, "w"))) {
        perror("Couldn't open file fp at AACO\n");
        exit(1);
    }
    /********** Main loop **********/
