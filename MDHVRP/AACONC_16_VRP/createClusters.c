...
void createClusters(int *K, SON *G, int vi, int n_size, int n_prim, int n_sect, int *da_access){
    ...
    for (int j = 0; j < n_sect; j++){
        ...
        if(closest != -1){
            if(closest > G->n_customers || da_access[closest - 1] == 0){printf("Error in createCluster\n"); exit(1);}
...
/* Continue filling cluster 1 and then the rest */ //(n_sect<n_size)
    while(not_empty_clusters(v_free, G->n_customers, da_access)){
        ...
        if(closest != -1){
            if(closest > G->n_customers || da_access[closest - 1] == 0){printf("Error in createCluster2\n"); exit(1);}
