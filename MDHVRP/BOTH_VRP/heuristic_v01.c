...
void heuristic_v01(SON *G, VType *VT, asolution *R, int **da_access, int *remaining){
    ...
    //If customer or depot not in cluster, adj_matrix[node]->id == -1
            ...
            for(int t = 1; t < G->n_differentTypes; t++){
                ...
                while(index_nn < nn_length - 1){
                    ...
                    if(successive_nodes && listLength(successive_nodes) == capacity){
                        route1 = copyList(successive_nodes);
                        push(&route1, G->a_depots[IDEPOT].id);
                        append(&route1, G->a_depots[IDEPOT].id);

                        node *l1 = copyList(R->a_VT[type].a_depots[IDEPOT].routelist);
                        node *l2 = copyList(route1);
