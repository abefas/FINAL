...
void AACONC(SON *G, VType *VT, int **da_access, int n_ants, int n_freq, int n_size, int n_sect, int n_sect_M, 
            int n_prim, double T_update, double a_update, double p_min, double p_max, double d, double a, double b)
{
...
/* Initialize 4-D pheromone array */
    ...
    for(int ivt = 0; ivt < G->n_differentTypes; ivt++){
        ...
        if(ivt == 2){
            ...
            for(int i = G->n_customers; i < (G->n_nodes); i++){
                K[ivt][i] = calloc(rows * n_size, sizeof *K[ivt][i]);
                if(!K[ivt][i]){perror("Error callocing K[ivt][i] clusters..\n"); exit(1);}
                //createClusters(K[ivt][i], G, i, n_size, n_prim, n_sect_D, da_access[ivt]);
            ...
            for(int i = 0; i < (G->n_nodes); i++){
                K[ivt][i] = calloc(rows * n_size, sizeof *K[ivt][i]);
                if(!K[ivt][i]){perror("Error callocing K[ivt][i] clusters..\n"); exit(1);}
                createClusters(K[ivt][i], G, i, n_size, n_prim, n_sect_M, da_access[ivt]);
                counter++;
            }
        }else{
            for(int i = 0; i < (G->n_nodes); i++){
    ...
    int *edge_matrix, *edges;
    if(NULL == (edge_matrix = malloc(sizeof *edge_matrix * G->n_differentTypes*G->n_nodes*G->n_nodes)) ||
        NULL == (edges = malloc(sizeof *edges * G->n_differentTypes))){
    ...
    /********** Main loop **********/
    while(stop_cond < 10000 && iter < 1000000 && term_condition > (0.00 - epsilon) && loop_time < 3600)
    {
        R_best.total_makespan = HUGE_VAL;
        int edge_sum = 0;
        memset(edge_matrix, 0, G->n_differentTypes*(G->n_nodes)*(G->n_nodes) * sizeof *edge_matrix);
        memset(edges, 0, G->n_differentTypes * sizeof *edges);
        ...
        for(int i = 0; i < n_ants; i++){
            ...
            edge_sum += store_edge_count(&Ra_local, G, edge_matrix, edges);
        ...
        term_condition = evaporate_pheromones(G, edge_matrix, phMatrix, edge_sum, n_ants, edges, p_min, p_max);
