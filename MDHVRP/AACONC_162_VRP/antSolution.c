...
void antSolution(SON *G, VType *VT, int ***K, double *phMatrix, asolution *Ra, int **da_access, 
    ...
    launch_count[G->n_depots][G->n_differentTypes], 
    served_count[G->n_depots][G->n_differentTypes];

    for(int idep = 0; idep < G->n_depots; idep++){
        for(int i = 0; i < G->n_differentTypes; i++)
            launch_count[idep][i] = served_count[idep][i] = 1;
    }
    ...
    { 
        ...
        while(ivt == -1 || idepot == -1 || icluster == -1 || icustomer == -1){
            ...
            ivt = selectVehicleType(idepot, Ra, VT, v_free, G, K, phMatrix, launch_count[idepot], served_count[idepot], da_access, n_size, n_prim);
            if(ivt == -1)
                continue;
            ...
            if(icustomer >= G->n_customers){
                printf("ivt %d idepot %d icluster %d icustomer %d\n", ivt, idepot, icluster, icustomer);
                exit(1);
            }
