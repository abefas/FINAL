            ...
            if(G->a_depots[idep].n_VT[ivt] != 0){
                ...
                if(ivt != 2){
                    while(temp->next){
                        ...
                        edge_matrix[(ivt*G->n_nodes*G->n_nodes) + (G->n_nodes)*(temp->data - 1) + (temp->next->data - 1)] += 1;
                        //edge_matrix[(ivt*G->n_nodes*G->n_nodes) + (temp->data - 1) + (G->n_nodes)*(temp->next->data - 1)] += 1;
                        edges[ivt]++;
                        edge_sum++;
...
double evaporate_pheromones(SON *G, int *edge_matrix, double *phMatrix, int edge_sum, int n_ants, int *edges, double p_min, double p_max){
    ...
    double H[G->n_differentTypes], *p;
    for(int i = 0; i < G->n_differentTypes; i++)
        H[i] = 0.0;

    if(NULL == (p = calloc(G->n_differentTypes*G->n_nodes*G->n_nodes, sizeof *p))){
    ...
    for(int ivt = 0; ivt < G->n_differentTypes; ivt++){
        for(int i = 1; i < G->n_nodes; i++){
            for(int j = 0; j < i; j++){
                if(i != j && edges[ivt] > 0){
                    p[(ivt*G->n_nodes*G->n_nodes) + (G->n_nodes)*j + i] = 
                        (double) edge_matrix[(ivt*G->n_nodes*G->n_nodes) + (G->n_nodes)*j + i] / edges[ivt] +
                        (double) edge_matrix[(ivt*G->n_nodes*G->n_nodes) + (G->n_nodes)*i + j] / edges[ivt];
                }
            }
        }
    }
    ...
    for(int ivt = 0; ivt < G->n_differentTypes; ivt++){
        for(int i = 1; i < G->n_nodes; i++){
            for(int j = 0; j < i; j++){
                if(i != j && p[(ivt*G->n_nodes*G->n_nodes) + (G->n_nodes*j) + i] > 0){
                    H[ivt] -= 
                        p[(ivt*G->n_nodes*G->n_nodes) + (G->n_nodes)*j + i] * log2(p[(ivt*G->n_nodes*G->n_nodes) + (G->n_nodes)*j + i]);
                }
            }
        }
    }
    ...
    free(p);
    double H_min[G->n_differentTypes], H_max[G->n_differentTypes];
    for(int ivt = 0; ivt < G->n_differentTypes; ivt++){
        H_min[ivt] = -log2((double)n_ants/edges[ivt]);
        H_max[ivt] = -log2(1.0/edges[ivt]);
    }

    double rho[G->n_differentTypes];// = p_min + (p_max - p_min)*((H - H_min)/(H_max - H_min));
    for(int ivt = 0; ivt < G->n_differentTypes; ivt++){
        rho[ivt] = p_min + (p_max - p_min)*((H[ivt] - H_min[ivt]) / (H_max[ivt] - H_min[ivt]));
    }
            ...
            if(G->a_depots[idep].n_VT[ivt] != 0){
                ...
                while(temp->next){
                    ...
                    if(temp->next->data > G->n_customers){
                        ...
                        fprintf(fp, "type %d depot %4d route %4d q_served %4d time %5.2lf\t", ivt+1, idep+G->n_customers+1, vehicle, q_served, v_ms);
                        fprintf(fp_1, "%d,%d\n", temp->data, temp->next->data);
