...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < G.n_differentTypes; i++) {
        if (NULL == (da_access[i] = calloc(G.n_customers, sizeof *da_access[i]))) {
    ...
    for (int i = 0; i < G.n_depots; i++) {
        if (NULL == (G.a_depots[i].n_VT = malloc(sizeof *G.a_depots[i].n_VT * G.n_differentTypes))) {
        ...
        for (int ivt = 0; ivt < G.n_differentTypes; ivt++) {
            VT[ivt].n_vehicles +=
                G.a_depots[i].n_VT[ivt]; /* Get number of vehicles of this type */

            if (G.a_depots[i].n_VT[ivt] != 0) /* Get number of depots for each Vehicle Type */
                VT[ivt].n_depots++;
        }
    ...
    for (int i = 0; i < 100; i++){
        srand(time(NULL));
        AACONC(&G, VT, da_access, n_ants, n_freq, n_size, n_sect, n_sect_M, n_prim, T_update,
