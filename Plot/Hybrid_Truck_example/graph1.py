...
ivt_0 = pd.read_csv("p07-15-15-15_run-5_ivt_0.csv")  # Truck paths
ivt_1 = pd.read_csv("p07-15-15-15_run-5_ivt_1.csv")  # Motorbike paths
ivt_2 = pd.read_csv("p07-15-15-15_run-5_ivt_2.csv")  # Drone paths
...
def plot_paths(df, line_color, marker_color, linestyle, label, special_handling=False):
    for i, row in df.iterrows():
        ...
        start_coord = node_coords.get(row['StartNode'])
        end_coord = node_coords.get(row['EndNode'])
        ...
        if start_coord and end_coord:
            ...
            if special_handling and ((start_node in depot_nodes and end_node in customer_nodes) or 
                ...
                plt.plot(
                    ...
                    markeredgecolor=marker_color, markersize=6, linewidth=1,
                ...
                plt.plot(
                    ...
                    markerfacecolor='none', markeredgecolor=marker_color, markersize=6,
                    linewidth=1, label=label if i == 0 else ""
                )

            # Plot start node
            if start_node in depot_nodes:
                plt.plot(start_coord[0], start_coord[1], 'o', markersize=6, markerfacecolor='#8B0000', markeredgecolor='#8B0000')
            else:
                plt.plot(start_coord[0], start_coord[1], 'o', markersize=6, markerfacecolor='none', markeredgecolor=marker_color, markeredgewidth=0.5)
                
            # Plot end node
            if end_node in depot_nodes:
                plt.plot(end_coord[0], end_coord[1], 'o', markersize=6, markerfacecolor='#8B0000', markeredgecolor='#8B0000')
            else:
                plt.plot(end_coord[0], end_coord[1], 'o', markersize=6, markerfacecolor='none', markeredgecolor=marker_color, markeredgewidth=0.5)
...
# Plot truck paths with solid red lines and red nodes
plot_paths(ivt_0, line_color='#FFA07A', marker_color='#FFA07A', linestyle='-', label='$G_T$')

# Plot motorbike paths with dashed gray lines and green nodes
plot_paths(ivt_1, line_color='green', marker_color='green', linestyle='-', label='$G_M$')

# Plot drone paths with dashed gray lines and blue nodes
plot_paths(ivt_2, line_color='gray', marker_color='blue', linestyle='--', label='$G_D$')
...
plt.legend(loc='upper right', fontsize=10, frameon=True, fancybox=True, shadow=False, edgecolor='black')

# Set axis labels and title
plt.xlabel("X Coordinate")
plt.ylabel("Y Coordinate")
plt.title("Truck, Motorbike, and Drone Routes")
