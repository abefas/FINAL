...
cluster_colors = ['orange', 'green', 'blue', 'purple', 'magenta', 'cyan', 'lightblue']
vehicle_colors = {'truck': '#FFA07A', 'motorbike': '#20B2AA', 'drone': 'gray'}
...
for cluster in set(clusters.values()):
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=nodes, node_color=cluster_colors[cluster % len(cluster_colors)],
                           node_size=100, edgecolors='black', label=f'Cluster {cluster}', alpha=0.9)
...
nx.draw_networkx_nodes(G, pos, nodelist=depot_nodes, node_color='#8B0000', node_size=200, label='Depot', edgecolors='black')
...
for vehicle, color in vehicle_colors.items():
    ...
    nx.draw_networkx_edges(G, pos, edgelist=edges, edge_color=color, width=1.5, style=style, label=f'{vehicle.capitalize()} route', alpha=0.7)
...
plt.ylim(0, 3500)  # Set y-axis from 0 to 4000 units
