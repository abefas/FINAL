...
data = pd.read_csv('heuristics_gaps_to_WORST.csv')
...
plt.title('Comparison between the heuristic and the AACONC+ worst', fontsize=16, weight='bold')
plt.legend(title='Algorithm', fontsize=12)
plt.xticks(rotation=45, fontsize=12)

# Set y-axis interval of 5
y_min = int(data.iloc[:, 1:].min().min() // 5 * 5)  # Find minimum y value rounded down to nearest multiple of 5
y_max = int(data.iloc[:, 1:].max().max() // 5 * 5 + 5)  # Find maximum y value rounded up to nearest multiple of 5
