...
%	\par 
	...
	To report on the experimental evaluation of the proposed algorithms we relied on 16 different instances, up to 1000 customers. The first 11 involve up to 249 customers and up to 5 depots and are taken from the MDVRP Cordeau \{ADD REF\} instances, modified to accomodate for the differences between the two problems while keeping the topology intact. To resemble a realistic scenario, the coordinate values of the nodes where multiplied by 50, such as to simulate real-life distances in cities or city centers. A more detailed description is provided in/Exact dimensions are shown in \{ADD TABLE of instances\}. Instances 12 to 16 are newly created and involve 500 to 1000 customers which were randomly placed, and 4 to 6 depots which were evenly distributed across the map.
